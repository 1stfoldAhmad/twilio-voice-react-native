...
export class Voice extends EventEmitter {
    ...
    async _connect_android(token, params, notificationDisplayName) {
        const connectResult = await NativeModule.voice_connect_android(token, params, notificationDisplayName)
            .then((callInfo) => {
            return { type: 'ok', callInfo };
        })
            .catch((error) => {
            const code = error.userInfo.code;
            const message = error.userInfo.message;
            return { type: 'err', message, code };
        });
     ...
     */
    _handleNativeEvent = (nativeVoiceEvent) => {
     ...
     * @param nativeVoiceEvent - A `Voice` event directly from the native layer.
     */
    _handleCallInvite = (nativeVoiceEvent) => {
        if (nativeVoiceEvent.type !== Constants.VoiceEventTypeValueIncomingCallInvite) {
            throw new Error('Incorrect "voice#callInvite" handler called for type ' +
     ...
     * @param nativeVoiceEvent - A `Voice` event directly from the native layer.
     */
    _handleError = (nativeVoiceEvent) => {
        if (nativeVoiceEvent.type !== Constants.VoiceEventError) {
            throw new Error('Incorrect "voice#error" handler called for type ' +
                `"${nativeVoiceEvent.type}".`);
        }
        const { error: { code, message }, } = nativeVoiceEvent;
     ...
     */
    _handleRegistered = (nativeVoiceEvent) => {
     ...
     */
    _handleUnregistered = (nativeVoiceEvent) => {
     ...
     * @param nativeVoiceEvent - A `Voice` event directly from the native layer.
     */
    _handleAudioDevicesUpdated = (nativeVoiceEvent) => {
        if (nativeVoiceEvent.type !== Constants.VoiceEventAudioDevicesUpdated) {
            throw new Error('Incorrect "voice#audioDevicesUpdated" handler called for type ' +
                `"${nativeVoiceEvent.type}".`);
        }
        const { audioDevices: audioDeviceInfos, selectedDevice: selectedDeviceInfo, } = nativeVoiceEvent;
        const audioDevices = audioDeviceInfos.map((audioDeviceInfo) => new AudioDevice(audioDeviceInfo));
        const selectedDevice = typeof selectedDeviceInfo !== 'undefined' && selectedDeviceInfo !== null
     ...
     */
    async connect(token, { contactHandle = 'Default Contact', notificationDisplayName = undefined, params = {}, } = {}) {
        if (typeof token !== 'string') {
            throw new InvalidArgumentError('Argument "token" must be of type "string".');
        }
        if (typeof contactHandle !== 'string') {
            throw new InvalidArgumentError('Optional argument "contactHandle" must be undefined or of type' +
                ' "string".');
        }
    ...
    async getCalls() {
        ...
        const callsMap = new Map(callInfos.map((callInfo) => [
            callInfo.uuid,
            new Call(callInfo),
        ]));
        return callsMap;
    ...
    async getCallInvites() {
        ...
        const callInvitesMap = new Map(callInviteInfos.map((callInviteInfo) => [
            callInviteInfo.uuid,
    ...
    async getAudioDevices() {
        const { audioDevices: audioDeviceInfos, selectedDevice: selectedDeviceInfo, } = await NativeModule.voice_getAudioDevices();
        const audioDevices = audioDeviceInfos.map((audioDeviceInfo) => new AudioDevice(audioDeviceInfo));
        const selectedDevice = typeof selectedDeviceInfo !== 'undefined'
            ? new AudioDevice(selectedDeviceInfo)
            : undefined;
